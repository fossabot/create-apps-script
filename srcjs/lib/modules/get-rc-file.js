...
async function index(directory) {
	...
	if (npmrc && yarnrc) {
		...
		console.group(
			chalk.yellow("No .npmrc or .yarnrc file detected. Searched paths:")
		)
		// Console.warn();
		...
		console.info(
			chalk.grey(
				"To take advantage of npm or yarn's default init options, create the appropriate rc file.",
				"\n"
			)
		)
